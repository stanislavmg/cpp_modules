...
void Bureaucrat::signForm(AForm &AForm) {
  try {
    AForm.beSigned(*this);
    std::cout << name_ << " signs " << AForm.getName() << std::endl;
  } catch (std::exception &e) {
    std::cout << name_ << " cannot sign " << AForm.getName() << " because "
              << e.what() << std::endl;
  }
}
...
void Bureaucrat::executeForm(AForm &AForm) {
  try {
    AForm.execute(*this);
    std::cout << name_ << " executes " << AForm.getName() << std::endl;
  } catch (std::exception &e) {
    std::cout << name_ << " cannot execute " << AForm.getName() << " because "
              << e.what() << std::endl;
  }
}
